...
public class SystemPropertyHelper {
	...
	public static String getElementScreenShotValue() {
		...
		String valueFromPropFile = PropertyReader.getInstance().getProperty(PropKey.ELEMENT_SCREEN_SHOT.getPropVal());

		String valueFromSystemVariable = System.getProperty("elementScreenShot");// kommmt aus der Pom Properties
		...
		msg = "No system property specified" + " ...defaulting to application.properties file ->  "
				+ PropKey.ELEMENT_SCREEN_SHOT.getPropVal() + " : " + valueFromPropFile;
	...
	public static String getRemoteVal() {
		...
		String msg = "";
		String valueFromPropFile = PropertyReader.getInstance().getProperty(PropKey.REMOTE.getPropVal());

		String remoteVal = System.getProperty(PropKey.REMOTE.getPropVal());

		if (remoteVal.length() > 0) {
			if (remoteVal.equalsIgnoreCase("TRUE") || remoteVal.equalsIgnoreCase("FALSE")) {

				return remoteVal;
			} else {

				msg = "Incorrect system property " + remoteVal + " value specified for" + PropKey.REMOTE.getPropVal()
						+ " ...defaulting to property provided by the application.properties file "
